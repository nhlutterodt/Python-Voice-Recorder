...
class PerformanceBenchmark:
    ...
    def generate_report(self) -> str:
        ...
        for operation_name, metrics in operations.items():
            ...
            memory_deltas = [m.memory_delta / (1024 * 1024) for m in metrics]  # Convert to MB
            
            report.append(f"  Count: {len(metrics)}")
            report.append(f"  Duration - Avg: {sum(durations)/len(durations):.3f}s, Min: {min(durations):.3f}s, Max: {max(durations):.3f}s")
            report.append(f"  Memory - Avg: {sum(memory_deltas)/len(memory_deltas):+.1f}MB, Min: {min(memory_deltas):+.1f}MB, Max: {max(memory_deltas):+.1f}MB")
            
            # Show recent operations
            recent_metrics = sorted(metrics, key=lambda x: x.timestamp, reverse=True)[:3]
            report.append("  Recent Operations:")
            for i, metric in enumerate(recent_metrics, 1):
                report.append(f"    {i}. {metric.timestamp.strftime('%H:%M:%S')} - {metric.duration:.3f}s ({metric.memory_delta/(1024*1024):+.1f}MB)")
        ...
        report.append(f"  Average Operation Duration: {sum(all_durations)/len(all_durations):.3f}s")
        report.append(f"  Memory Usage - Avg: {memory_stats['avg']:.1f}MB, Peak: {memory_stats['max']:.1f}MB")
    ...
    def export_csv(self, filename: str = None) -> str:
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['timestamp', 'operation', 'duration_s', 'memory_delta_mb', 'additional_data']
            ...
            for metric in self.metrics:
                writer.writerow({
                    ...
                    'additional_data': str(metric.additional_data) if metric.additional_data else ''
                })
...
def benchmark_operation(operation_name: str, **additional_data):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            with performance_monitor.measure_operation(operation_name, **additional_data):

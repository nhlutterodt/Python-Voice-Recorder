...
def test_upload_raises_on_duplicate_when_not_forced(monkeypatch):
    ...
    fake_manager.find_duplicate_by_content_sha256 = Mock(return_value={'id': 'file123', 'name': 'existing.wav'})

    # Patch the chunked upload helper so we can assert it wasn't called
    mock_chunked = Mock()
    monkeypatch.setattr('cloud.google_uploader.chunked_upload_with_progress', mock_chunked)
...
def test_upload_proceeds_when_forced(monkeypatch):
    ...
    def fake_chunked(create_request, progress_callback=None, cancel_check=None):
        ...
        try:
            ...
            raise
        return {'id': 'uploaded-file-789', 'name': 'uploaded.wav', 'size': '12345', 'createdTime': 'now'}

    monkeypatch.setattr('cloud.google_uploader.chunked_upload_with_progress', fake_chunked)

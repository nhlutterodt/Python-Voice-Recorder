...
def worker_main(db_path: str, out_q: multiprocessing.Queue, sleep_ms: float = 0.0, fail_dir: Optional[str] = None) -> None:
...
def _run_workers_and_collect(tmp_path: Path, db_path: str, worker_count: int, **worker_kwargs):
    ctx = multiprocessing.get_context("spawn")
    out_q: multiprocessing.Queue = ctx.Queue()
    procs = [ctx.Process(target=worker_main, args=(db_path, out_q, worker_kwargs.get('sleep_ms', 0.0), worker_kwargs.get('fail_dir')))
...
def test_atomic_claims_under_parallel_access_stress(tmp_path: Path) -> None:
    ...
    assert len(unique_ids) == total, f"expected {total} unique processed ids, got {len(unique_ids)}"

    # verify final state for each job is 'completed'
    q2 = SqliteJobQueue(db_path=db_path)
    for jid in unique_ids:
        rec = q2.get_job(jid)
        assert rec is not None and rec.status == 'completed', f"job {jid} final status is not completed"
...
def test_retries_under_concurrency(tmp_path: Path) -> None:
    ...
    received = _run_workers_and_collect(tmp_path, db_path, worker_count=4, sleep_ms=5, fail_dir=str(fail_dir))

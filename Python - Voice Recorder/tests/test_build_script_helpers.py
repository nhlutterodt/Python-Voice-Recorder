...
class TestBuildScriptHelpers(unittest.TestCase):
    ...
    def test_check_package_import_failure(self):
        ...
        success, is_pydub_audioop = self.builder._check_package_import(
            "nonexistent", "nonexistent_package_12345"
        )
    ...
    def test_check_package_import_pydub_audioop_issue(self):
        ...
        with patch(
            "builtins.__import__", side_effect=ImportError("No module named 'audioop'")
        ):
            success, is_pydub_audioop = self.builder._check_package_import(
                "pydub", "pydub"
            )

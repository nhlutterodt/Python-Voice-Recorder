...
def test_start_job_worker_starts_thread(monkeypatch):
    ...
    def fake_run_worker_with_supervisor(
        drive_manager, db_path=None, poll_interval=2, stop_event=None
    ):
    ...
    fake_config = types.SimpleNamespace(
        enable_cloud_job_worker=True, cloud_jobs_db_path=None
    )
    ...
    monkeypatch.setattr(
        "cloud.job_queue_sql.run_worker_with_supervisor",
        fake_run_worker_with_supervisor,
        raising=False,
    )

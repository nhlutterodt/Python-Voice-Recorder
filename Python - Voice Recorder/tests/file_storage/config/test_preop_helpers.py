...
def test_validate_before_operation_sufficient_capacity(monkeypatch, tmp_path):
    ...
    cfg = ConstraintConfig(min_disk_space_mb=10, max_file_size_mb=1000, enable_disk_space_check=True, retention_days=30)
    ...
    monkeypatch.setattr('services.file_storage.config.constraints._evaluate_capacity_for_operation', fake_evaluator)
...
def test_validate_before_operation_insufficient_capacity(monkeypatch, tmp_path):
    cfg = ConstraintConfig(min_disk_space_mb=50, max_file_size_mb=1000, enable_disk_space_check=True, retention_days=30)
    sc = StorageConstraints(cfg)
    cv = ConstraintValidator(sc)
    def fake_evaluator(constraints, base_path, estimated_size_mb):
        # simulate insufficient free space
        return False, ['Not enough space'], ['low'], ['cleanup'], {'valid': False, 'capacity_info': {'free_mb': 10.0}, 'errors': ['insufficient']}

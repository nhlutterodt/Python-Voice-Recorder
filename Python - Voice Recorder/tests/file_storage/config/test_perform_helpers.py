...
def test_perform_source_file_validation_delegates(monkeypatch):
    ...
    monkeypatch.setattr(constraints_mod, '_maybe_validate_source_file', lambda s, c: fake)
...
def test_perform_capacity_preop_check_delegates(monkeypatch, tmp_path):
    fake_tuple = (True, [], [], [], {'valid': True, 'capacity_info': {'free_mb': 500.0}})
    monkeypatch.setattr(constraints_mod, '_evaluate_capacity_for_operation', lambda c, b, e: fake_tuple)

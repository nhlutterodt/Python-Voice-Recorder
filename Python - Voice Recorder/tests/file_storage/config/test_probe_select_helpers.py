...
def test_get_candidate_collectors_prefers_module_level_mock(monkeypatch):
    ...
    monkeypatch.setitem(constraints.__dict__, 'StorageInfoCollector', mock.MagicMock(name='ModuleLevelMock'))
    # Patch import inside function by inserting a module object into package
    monkeypatch.setitem(constraints.__package__ and __import__(constraints.__package__).__dict__, 'storage_info', fake_storage_info_mod)
...
def test_probe_candidate_collectors_returns_valid_candidates(tmp_path):
    ...
    assert method_name in ('get_raw_storage_info', 'get_disk_usage', 'get_storage_info', '__call__')

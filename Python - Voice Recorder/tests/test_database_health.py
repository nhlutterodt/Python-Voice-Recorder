...
class TestDatabaseHealthMonitor:
    ...
    def test_health_monitor_with_callback_initialization(
        self, health_monitor_with_callback
    ):
    ...
    @patch("core.database_context.db_context")
    @patch("time.time")
    def test_query_performance_check_fast(
        self, mock_time, mock_db_context, health_monitor
    ):
    ...
    @patch("core.database_context.db_context")
    @patch("time.time")
    def test_query_performance_check_slow(
        self, mock_time, mock_db_context, health_monitor
    ):
    ...
    def test_run_health_checks(self, health_monitor):
        ...
        with (
            patch.object(health_monitor, "_check_database_connectivity") as mock_conn,
            ...
            mock_conn.return_value = HealthCheckResult(
                "connectivity", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
            mock_perf.return_value = HealthCheckResult(
                "performance", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
            mock_disk.return_value = HealthCheckResult(
                "disk", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
            mock_memory.return_value = HealthCheckResult(
                "memory", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
            mock_table.return_value = HealthCheckResult(
                "table", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
            mock_sqlite.return_value = HealthCheckResult(
                "sqlite", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
            mock_wal.return_value = HealthCheckResult(
                "wal", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
            mock_vacuum.return_value = HealthCheckResult(
                "vacuum", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
            )
    ...
    def test_get_health_status(self, health_monitor):
        ...
        with (
            patch.object(health_monitor, "_run_health_checks") as mock_checks,
            patch.object(health_monitor, "_get_enhanced_engine_info") as mock_engine,
            patch.object(
                health_monitor, "_get_enhanced_performance_metrics"
            ) as mock_metrics,
            patch.object(
                health_monitor, "_get_system_resource_status"
            ) as mock_resources,
        ):

            mock_checks.return_value = [
                HealthCheckResult(
                    "test1", HealthCheckStatus.PASS, HealthCheckSeverity.INFO
                ),
                HealthCheckResult(
                    "test2", HealthCheckStatus.WARNING, HealthCheckSeverity.WARNING
                ),
            ]
    ...
    def test_alert_callback_not_triggered_for_normal_issues(
        self, health_monitor_with_callback
    ):
    ...
    def test_health_trend_analysis_with_data(self, health_monitor):
        ...
        for i in range(10):
            health_monitor.health_history.append(
                {"timestamp": time.time() - (i * 60), "overall_status": "healthy"}
            )
    ...
    def test_backward_compatibility_methods(self, health_monitor):
        ...
        with (
            patch.object(
                health_monitor, "_get_enhanced_engine_info"
            ) as mock_enhanced_engine,
            patch.object(
                health_monitor, "_get_enhanced_performance_metrics"
            ) as mock_enhanced_perf,
        ):
...
class TestHealthMonitoringIntegration:
    ...
    def test_unhealthy_system_monitoring(self):
        ...
        with (
            ...
            assert any(
                "failed health checks" in rec.lower()

...
def enqueue_job(job: JobRow, db_path: Optional[str] = None) -> None:
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT OR REPLACE INTO jobs (id,file_path,title,description,tags,status,attempts,max_attempts,created_iso,started_iso,finished_iso,uploaded_bytes,total_bytes,last_error,drive_file_id,cancel_requested) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (
            job.id,
            job.file_path,
            job.title,
            job.description,
            json.dumps(job.tags) if job.tags is not None else None,
            job.status,
            job.attempts,
            job.max_attempts,
            job.created_iso or datetime.utcnow().isoformat(),
            job.started_iso,
            job.finished_iso,
            job.uploaded_bytes,
            job.total_bytes,
...
def dequeue_next(db_path: Optional[str] = None) -> Optional[JobRow]:
    ...
    cur.execute(
        "SELECT * FROM jobs WHERE (status='pending' OR (status='failed' AND attempts<max_attempts)) ORDER BY created_iso LIMIT 1"
    )
    row = cur.fetchone()
    if not row:
        conn.close()
    ...
    cur.execute(
        "UPDATE jobs SET status='running', attempts=attempts+1, started_iso=? WHERE id=?",
        (started, row["id"]),
    )
    conn.commit()
    conn.close()
...
def update_job_status(
    ...
    if finished is not None:
        cur.execute(
            "UPDATE jobs SET status=?, last_error=?, drive_file_id=?, finished_iso=? WHERE id=?",
            (status, last_error, drive_file_id, finished, job_id),
        )
    else:
        cur.execute(
            "UPDATE jobs SET status=?, last_error=?, drive_file_id=? WHERE id=?",
            (status, last_error, drive_file_id, job_id),
        )
    conn.commit()
    conn.close()
...
def update_job_progress(
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE jobs SET uploaded_bytes=?, total_bytes=? WHERE id=?",
        (uploaded_bytes, total_bytes, job_id),
    )
    conn.commit()
    conn.close()
        ...
        try:
            ...
            try:
                ...
                if cancel_event.is_set():
                    ...
                    update_job_status(
                        job.id, "failed", db_path=db_path, last_error=str(e)
                    )
        except Exception as e:
...
def run_worker_with_supervisor(
    drive_manager,
    db_path: Optional[str] = None,
    poll_interval: float = 2.0,
    stop_event: Optional[threading.Event] = None,

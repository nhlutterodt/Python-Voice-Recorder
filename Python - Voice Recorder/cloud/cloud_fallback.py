...
class CloudFallbackWidget(QWidget):
    ...
    def init_ui(self):
        ...
        hint = QLabel('Install dev/cloud requirements and add a Google client_secrets.json to enable sign-in.')
    ...
    def on_retry(self):
        try:
            ...
            QMessageBox.warning(self, 'Retry Failed', f'Failed to initialize cloud features: {e}')

    def on_open_requirements(self):
        try:
            path = os.path.join(os.path.dirname(__file__), '..', 'requirements_cloud.txt')
            abs_path = os.path.abspath(path)
            if os.path.exists(abs_path):
                QDesktopServices.openUrl(QUrl.fromLocalFile(abs_path))
            else:
                QMessageBox.information(self, 'Not Found', f'Requirements file not found: {abs_path}')
        except Exception as e:
